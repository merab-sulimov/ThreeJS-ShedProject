...
//
//     let cube = CSG.cube();
//     let sphere = CSG.sphere({ radius: 1.3 });
//     let polygons = cube.subtract(sphere).toPolygons();
//
...
CSG.cube = function (options) {
    options = options || {};
    let c = new CSG.Vector(options.center || [0, 0, 0]);
    let r = !options.radius ? [1, 1, 1] : options.radius.length ?
...
CSG.sphere = function (options) {
    options = options || {};
    let c = new CSG.Vector(options.center || [0, 0, 0]);
    let r = options.radius || 1;
    let slices = options.slices || 16;
    let stacks = options.stacks || 8;
    let polygons = [];
    let vertices;
...
CSG.cylinder = function (options) {
    options = options || {};
    let s = new CSG.Vector(options.start || [0, -1, 0]);
    let e = new CSG.Vector(options.end || [0, 1, 0]);
    let ray = e.minus(s);
    let r = options.radius || 1;
    let slices = options.slices || 16;
    let axisZ = ray.unit();
    let isY = (Math.abs(axisZ.y) > 0.5);
    let axisX = new CSG.Vector(isY, !isY, 0).cross(axisZ).unit();
    let axisY = axisX.cross(axisZ).unit();
    let start = new CSG.Vertex(s, axisZ.negated());
    let end = new CSG.Vertex(e, axisZ.unit());
    let polygons = [];
