...
class STLLoader {
    ...
    ensureBinary(buf) {
        if (typeof buf === "string") {
            let arrayBuffer = new Uint8Array(buf.length);
...
if (typeof DataView === 'undefined') {
    DataView = function (buffer, byteOffset, byteLength) { // eslint-disable-line no-global-assign
        this.buffer = buffer;
    ...
    DataView.prototype = {
        ...
        getFloat64: function (byteOffset, littleEndian) {
            ...
            let sign = 1 - (2 * (b[7] >> 7));
            let exponent = ((((b[7] << 1) & 0xff) << 3) | (b[6] >> 4)) - ((1 << 10) - 1);

            // Binary operators such as | and << operate on 32 bit values, using + and Math.pow(2) instead
            let mantissa = ((b[6] & 0x0f) * Math.pow(2, 48)) + (b[5] * Math.pow(2, 40)) + (b[4] * Math.pow(2, 32)) +
        ...
        getFloat32: function (byteOffset, littleEndian) {
            ...
            let sign = 1 - (2 * (b[3] >> 7));
            let exponent = (((b[3] << 1) & 0xff) | (b[2] >> 7)) - 127;
            let mantissa = ((b[2] & 0x7f) << 16) | (b[1] << 8) | b[0];
