...
class Control2D {
    ...
    constructor(camera, renderElement) {
        ...
        let targetPosition_ = {x: defaults_.x, z: defaults_.z};
        let targetZoom_ = defaults_.zoom;
        ...
        this.save = () => {
            ...
            targetZoom_ = camera.zoom;
        };

        this.restore = () => {
            camera.position.setX(targetPosition_.x);
            camera.position.setZ(targetPosition_.z);
            camera.zoom = targetZoom_;
            camera.updateProjectionMatrix();
        };
    }
}
