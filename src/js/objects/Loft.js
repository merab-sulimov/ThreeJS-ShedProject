...
class Loft extends DraggableObject {
    constructor(type, environmentCamera, shedHeight, parameters) {
        ...
        this.resize = (x, y, z) => {
            value_ = this.calculateValue(x, y, z);
        ...
        this.calculateValue = (x, y, z) => {
            let value;
            if (angle_ == 0) {
                value = parameters.shedDepth * 0.5 / ft1 - Math.round(z / ft1);
            } else {
                value = parameters.shedDepth * 0.5 / ft1 + Math.round(z / ft1);
            }
            if (value < 1) {
            ...
            z: {
                ...
                set: (value) => {
                    if (value < 0) {
                        ...
                        if (text_) {
                            text_.rotation.fromArray([0, Math.PI, 0])
                        }
                    } else {
                        value = parameters.shedDepth * 0.5;
                        angle_ = 0;
                        ...
                        cross_.position.x = grip_.position.x + 30;
                        if (text_) {
                            text_.rotation.fromArray([0, 0, 0])
                        }
                    }
        ...
        function draw() {
            ...
            grip_.position.y = 1;
            grip_.position.x = -parameters.shedWidth * 0.3;
            grip_.userData.target = self;
            grip_.userData.cursor = "ew-resize";
            container.add(grip_);
        ...
        function generateText() {
            ...
            text_ = new THREE.Mesh(new THREE.TextGeometry(value_ + `' ${self.type}`, {
                font: font_,
                size: 15,
                height: 0.5
