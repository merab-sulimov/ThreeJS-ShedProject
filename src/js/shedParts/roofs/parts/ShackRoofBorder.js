...
class ShackRoofBorder extends THREE.Object3D {
     ...
     * @param negativeDPadding Value on which the border should have padding against rootVertices on depth
     * @param negativeWPadding Value on which the border should have padding against rootVertices on width
     */
    constructor(roofVertices, negativeDPadding = 0, negativeWPadding = 0) {
        super();

        this.trimID = null;
        let bRoofGeometry = new THREE.BufferGeometry();

        if (negativeDPadding != 0 || negativeWPadding != 0) {
            let roofVector = new THREE.Vector3(roofVertices[0], roofVertices[1], roofVertices[2])
                .sub(new THREE.Vector3(roofVertices[3], roofVertices[4], roofVertices[5])).normalize();

            let roofAngle = roofVector.clone().angleTo(new THREE.Vector3(-1, 0, 0));
            negativeWPadding = negativeWPadding / Math.cos(roofAngle);
            let vector = new THREE.Vector3(0, 0, 0).sub(roofVector.clone().multiplyScalar(negativeWPadding));

            roofVertices = _.map(roofVertices, (vertex, i) => {
                if (i % 3 == 0) {
                    if (vertex > 0) {
                        return vertex + vector.x;
                    } else if (vertex < 0) {
                        return vertex - vector.x;
                    }
                } else if (i % 3 == 1) {
                    if (roofVertices[i - 1] != 0) {
                        return vertex - vector.y;
                    }
                } else if (i % 3 == 2) {
                    if (vertex > 0) {
                        return vertex + negativeDPadding;
                    } else if (vertex < 0) {
                        return vertex - negativeDPadding;
                    }
                }
        ...
        this.setColor = (mainColor, secondaryColor) => {
            ...
            return Promise.all([
                ...
                sb.material = trim.isMetal ? tools.PAINT_METAL_MATERIAL : tools.PAINT_MATERIAL;
